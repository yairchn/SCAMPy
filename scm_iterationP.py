...
def scm_iterP(ncore, myscampyfolder, true_data, myscampyfolder, theta,  case_name, fname, model_type, txt, geom_opt=0):
    ...
    namelist['stats_io']['frequency'] = 600.0# namelist['time_stepping']['t_max'] - can be set to lower frequency if data si too large 
    namelistfile.close()

    namelist['output']['output_root'] = '/scratch/' # depending on cluster - Ignacio
    ...
    #record_data(theta, u, new_data, new_dir, fname) - slows down the code
    os.remove(new_path)
...
def generate_costFun(theta, true_data,new_data, fname, model_type):
    ...
    #  - Ignacio
    # here you need to define the cost function for the tuning
    # this could be something like the differnece in BL height, cloud top , LWP , differnece in prognostic variable profiles (I called it CAPE_somevar see below)
    # in order to compare and take differnece betwen SCM and LES one has to be interpolated to the grid of the other
...
def create_record(theta_, costFun_, new_data, fname):
    ...
    if nnsim == 0.0:
        # commented out the matrixes to save computation - stor eonly the costfun and tune-parameter
