...
def generate_costFun(theta, true_data,new_data, fname, model_type):
    ...
    var_CF = np.sqrt(np.var(s_CF - p_CF, 0))  # (np.var(s_CF[ts1:], 0))
    var_CT = np.sqrt(np.var(s_CT - p_CT, 0))  # (np.var(s_CT[ts1:], 0))
    var_CT_temp = np.sqrt(np.var(s_CT_temp - p_CT_temp, 0))  # (np.var(s_CT[ts1:], 0))
    var_lwp = np.sqrt(np.var(s_lwp - p_lwp, 0))  # var_lwp = (np.var(s_lwp[ts1:], 0))
