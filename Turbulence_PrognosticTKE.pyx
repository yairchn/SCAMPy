...
cdef class EDMF_PrognosticTKE(ParameterizationBase):
    ...
    def __init__(self, namelist, paramlist, Grid Gr, ReferenceState Ref):
        ...
        try:
            ...
            elif str(namelist['turbulence']['EDMF_PrognosticTKE']['entrainment']) == 'tke':
                self.entr_detr_fp = entr_detr_tke
            elif str(namelist['turbulence']['EDMF_PrognosticTKE']['entrainment']) == 'tke2':
    ...
    cpdef initialize_io(self, NetCDFIO_Stats Stats):
        ...
        Stats.add_profile('upd_w_turb_flux')
        Stats.add_profile('upd_H_turb_flux')
        Stats.add_profile('upd_QT_turb_flux')

        if self.calc_tke:
            Stats.add_profile('env_tke_buoy')
            Stats.add_profile('env_tke_dissipation')
            Stats.add_profile('env_tke_entr_gain')
            ...
            Stats.add_profile('env_tke_transport')
            Stats.add_profile('env_tke_advection')

            Stats.add_profile('upd_tke_buoy')
            Stats.add_profile('upd_tke_dissipation')
            Stats.add_profile('upd_tke_entr_gain')
            ...
            Stats.add_profile('upd_tke_transport')
            Stats.add_profile('upd_tke_advection')
        ...
        if self.calc_scalar_var:
            ...
            Stats.add_profile('env_Hvar_entr_gain')
            Stats.add_profile('env_Hvar_turb_entr')
            Stats.add_profile('env_QTvar_entr_gain')
            Stats.add_profile('env_QTvar_turb_entr')
            Stats.add_profile('env_Hvar_detr_loss')
            Stats.add_profile('env_QTvar_detr_loss')
            Stats.add_profile('env_HQTcov_detr_loss')
            Stats.add_profile('env_HQTcov_entr_gain')
            ...
            Stats.add_profile('upd_Hvar_entr_gain')
            Stats.add_profile('upd_Hvar_turb_entr')
            Stats.add_profile('upd_QTvar_entr_gain')
            Stats.add_profile('upd_QTvar_turb_entr')
            Stats.add_profile('upd_Hvar_detr_loss')
            Stats.add_profile('upd_QTvar_detr_loss')
            Stats.add_profile('upd_HQTcov_detr_loss')
            Stats.add_profile('upd_HQTcov_entr_gain')
    ...
    cpdef io(self, NetCDFIO_Stats Stats):
        cdef:
            ...
            double [:] mean_upd_mixing_length = np.zeros((self.Gr.nzg,), dtype=np.double, order='c')
            double [:] mean_Eddy_diff_KH = np.zeros((self.Gr.nzg,), dtype=np.double, order='c')
            double [:] mean_Eddy_diff_KM = np.zeros((self.Gr.nzg,), dtype=np.double, order='c')
        ...
        Stats.write_profile('horizontal_KM', mean_horizontal_KM[self.Gr.gw:self.Gr.nzg-self.Gr.gw])
        Stats.write_profile('horizontal_KH', mean_horizontal_KH[self.Gr.gw:self.Gr.nzg-self.Gr.gw])
        Stats.write_profile('upd_w_turb_flux', mean_w_diff[self.Gr.gw:self.Gr.nzg-self.Gr.gw])
        Stats.write_profile('upd_H_turb_flux', mean_H_diff[self.Gr.gw:self.Gr.nzg-self.Gr.gw])
        Stats.write_profile('upd_QT_turb_flux', mean_QT_diff[self.Gr.gw:self.Gr.nzg-self.Gr.gw])
        Stats.write_profile('updraft_KH', mean_Eddy_diff_KH[self.Gr.gw:self.Gr.nzg-self.Gr.gw])
        Stats.write_profile('updraft_KM', mean_Eddy_diff_KM[self.Gr.gw:self.Gr.nzg-self.Gr.gw])
            ...
            for k in xrange(gw, self.Gr.nzg-gw):
                ...
                for i in range(self.n_updrafts):
                    ...
                    # Buoyancy-shear-subdomain exchange-dissipation TKE equilibrium scale
                        shear2 = pow((GMV.U.values[k+1] - GMV.U.values[k-1]) * 0.5 * self.Gr.dzi, 2) + \
                            pow((GMV.V.values[k+1] - GMV.V.values[k-1]) * 0.5 * self.Gr.dzi, 2) + \
                            pow((self.UpdVar.W.values[i,k] - self.UpdVar.W.values[i,k-1]) * self.Gr.dzi, 2)
                        ...
                        if self.UpdVar.QL.values[i,k] > 0.0:
                            d_buoy_thetal_total = (prefactor * (1.0 + eps_vi * (1.0 + lh / Rv / self.UpdVar.T.values[i,k] ) * (self.UpdVar.QT.values[i,k] - self.UpdVar.QL.values[i,k])  - self.UpdVar.QT.values[i,k] )
                                                     / (1.0 + lh * lh / cpm / Rv / self.UpdVar.QT.values[i,k] / self.UpdVar.T.values[i,k] * (self.UpdVar.QT.values[i,k] - self.UpdVar.QL.values[i,k]) ))
                            d_buoy_qt_total = (lh / cpm / self.UpdVar.QT.values[i,k]  * d_buoy_thetal_cloudy - prefactor) * self.UpdVar.H.values[i,k] 
                        else:
                            d_buoy_thetal_total = prefactor * (1.0 + (eps_vi-1.0) * self.UpdVar.QT.values[i, k])
                            d_buoy_qt_total = prefactor * self.UpdVar.H.values[i,k] * (eps_vi-1.0)
                        ...
                        # Production/destruction terms
                        a = self.tke_ed_coeff*(shear2 - grad_b_thl/self.prandtl_nvec[k] - grad_b_qt/self.prandtl_nvec[k])* sqrt(self.UpdVar.TKE.values[i,k])
                        # Dissipation term
                        c_neg = self.tke_diss_coeff*self.UpdVar.TKE.values[i,k]*sqrt(self.UpdVar.TKE.values[i,k])
                        # Subdomain exchange term
                        wc_upd = (self.UpdVar.W.values[nn,k] + self.UpdVar.W.values[nn,k-1])/2.0
                        wc_env = (self.EnvVar.W.values[k] - self.EnvVar.W.values[k-1])/2.0
                        bb = self.UpdVar.Area.values[nn,k]*wc_upd_nn*self.entr_sc[nn,k]/(1.0-self.UpdVar.Area.bulkvalues[k])*(
                                (wc_upd_nn-wc_env)*(wc_upd_nn-wc_env)/2.0-self.UpdVar.TKE.values[i,k])

                        if abs(a) > m_eps and 4.0*a*c_neg > - bb*bb:
                            self.l_entdet[k] = fmax( -bb/2.0/a + sqrt(bb*bb + 4.0*a*c_neg )/2.0/a, 0.0)
                        elif abs(a) < m_eps and abs(bb) > m_eps:
                            self.l_entdet[k] = c_neg/bb

                        l3 = self.l_entdet[k]

                        # Limiting stratification scale (Deardorff, 1976)
                        thv = theta_virt_c(self.Ref.p0_half[k], self.UpdVar.T.values[i,k], self.UpdVar.QT.values[i,k],
                            self.UpdVar.QL.values[i,k])
                        grad_thv_low = grad_thv_plus
                        grad_thv_plus = ( theta_virt_c(self.Ref.p0_half[k+1], self.UpdVar.T.values[i,k+1], self.UpdVar.QT.values[i,k+1],
                            self.UpdVar.QL.values[i,k+1]) - thv) * self.Gr.dzi
                        grad_thv = interp2pt(grad_thv_low, grad_thv_plus)
                        N = sqrt(fmax(g/thv*grad_thv, 0.0))
                        if N > 0.0:
                            l1 = fmin(sqrt(fmax(0.4*self.EnvVar.TKE.values[k],0.0))/N, 1.0e6)
                        ...
                        self.upd_mixing_length[0,k] = auto_smooth_minimum(l, 0.1)
                    else:
                        self.upd_mixing_length[0,k] = 0.0
            ...
            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                ...
                for i in xrange(self.n_updrafts):
                    if self.UpdVar.Area.values[i,k]>self.minimum_area:
                        ...
                        dw = (wu_half - we_half)
                        self.horizontal_KM[i,k] = 0.3*self.tke_ed_coeff*sqrt(fmax(GMV.TKE.values[k],0.0))*l
                        self.horizontal_KH[i,k] = 0.3*self.horizontal_KM[i,k] / self.prandtl_number
    ...
    cpdef compute_updraft_turb_flux(self, CasesBase Case):
        cdef:
            ...
            double dH = 0.0
            double dQT = 0.0
            double dw_low, dv_low, rho_au_Km_low, rho_au_Kh_full_low, K_full,
            double dw_high = 0.0
            double dH_high = 0.0
            double dQT_high = 0.0
            double rho_au_Km_high = 0.0
            double rho_au_Kh_full_high = 0.0
            double dzi = self.Gr.dzi
        ...
        for i in xrange(self.n_updrafts):
            ...
            for k in xrange(self.Gr.gw+2, self.Gr.nzg-self.Gr.gw):
                if self.UpdVar.Area.values[i,k]>self.minimum_area:
                    ...
                    rho_au_Km_low = self.Ref.rho0_half[k] * self.UpdVar.Area.values[i,k]*self.upd_KM[i,k]
                    rho_au_Km_high = self.Ref.rho0_half[k+1]* self.UpdVar.Area.values[i,k+1]*self.upd_KM[i,k+1]
                    dw_low = self.UpdVar.W.values[i,k] - self.UpdVar.W.values[i,k-1]
                    dw_high = self.UpdVar.W.values[i,k+1]- self.UpdVar.W.values[i,k]


                    rho_au_Kh_full_low = 0.5 * (self.Ref.rho0_half[k-1]* self.UpdVar.Area.values[i,k-1]*self.upd_KH[i,k-1]+
                                           self.Ref.rho0_half[k]* self.UpdVar.Area.values[i,k]*self.upd_KH[i,k])
                    rho_au_Kh_full_high = 0.5 * (self.Ref.rho0_half[k]* self.UpdVar.Area.values[i,k]*self.upd_KH[i,k]+
                                           self.Ref.rho0_half[k+1]* self.UpdVar.Area.values[i,k+1]*self.upd_KH[i,k+1])
                    dH_low = self.UpdVar.H.values[i,k] -  self.UpdVar.H.values[i,k-1]
                    dQT_low = self.UpdVar.QT.values[i,k] -  self.UpdVar.QT.values[i,k-1]
                    dH_high = self.UpdVar.H.values[i,k+1] - self.UpdVar.H.values[i,k]
                    ...
                    self.UpdVar.H.turb_flux[i,k] = 0.0
                    self.UpdVar.QT.turb_flux[i,k] = 0.0
    ...
    cpdef compute_nh_pressure_normalmodes(self):
        cdef:
            Py_ssize_t i, k
            double a_k, B_k, press_buoy, press_drag, coeff1, coeff2, coeff3, H, asp_ratio
            double whalf_k, whalf_kp

        coeff1 = 9.49539306e-01
        coeff2 = 2.62888475e-04
        coeff3 = 3.65369528e-01
                ...
                for k in range(gw, self.Gr.nzg-gw):
                    ...
                    if anew_k >= self.minimum_area:
                        ...
                        self.UpdVar.W.new[i,k] = (self.Ref.rho0[k] * a_k * self.UpdVar.W.values[i,k] * dti_
                                                  -adv + exch + buoy + self.nh_pressure[i,k] + self.UpdVar.W.turb_flux[i,k])/(self.Ref.rho0[k] * anew_k * dti_)
                ...
                for k in xrange(gw+1, self.Gr.nzg-gw):
                    ...
                    if self.UpdVar.Area.new[i,k] >= self.minimum_area:
                        ...
                        self.UpdVar.H.new[i,k] =  (c2 * self.UpdVar.H.values[i,k]  + c3 * self.UpdVar.H.values[i,k-1]
                                                   + c4 * H_entr + self.turb_entr_H[i,k] + self.UpdVar.H.turb_flux[i,k])/c1
                        self.UpdVar.QT.new[i,k] = (c2 * self.UpdVar.QT.values[i,k] + c3 * self.UpdVar.QT.values[i,k-1]
                                                   + c4* QT_entr + self.turb_entr_QT[i,k] + self.UpdVar.QT.turb_flux[i,k])/c1
                    else:
    ...
    cpdef compute_tke_pressure(self):
        cdef:
            Py_ssize_t k
            Py_ssize_t gw = self.Gr.gw
            double wu_half, we_half, press_half, a_half
        ...
        with nogil:
            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                ...
                for i in xrange(self.n_updrafts):
                    ...
                    self.EnvVar.TKE.press[k]  += (1.0-a_half) * (we_half - wu_half) * press_half
                    self.UpdVar.TKE.press[i,k] = a_half * (we_half - wu_half) * press_half
        return
    ...
    cpdef compute_covariance(self, GridMeanVariables GMV, CasesBase Case, TimeStepping TS):
        ...
        if self.calc_tke:
            ...
            self.compute_covariance_entr(self.EnvVar.TKE, self.UpdVar.TKE, self.UpdVar.W, self.UpdVar.W, self.EnvVar.W, self.EnvVar.W)
        ...
        if self.calc_scalar_var:
            self.compute_covariance_entr(self.EnvVar.Hvar, self.UpdVar.Hvar, self.UpdVar.H, self.UpdVar.H, self.EnvVar.H, self.EnvVar.H)
            self.compute_covariance_entr(self.EnvVar.QTvar, self.UpdVar.QTvar, self.UpdVar.QT, self.UpdVar.QT, self.EnvVar.QT, self.EnvVar.QT)
            self.compute_covariance_entr(self.EnvVar.HQTcov, self.UpdVar.HQTcov, self.UpdVar.H, self.UpdVar.QT, self.EnvVar.H, self.EnvVar.QT)
    ...
    cdef void compute_covariance_entr(self, EDMF_Environment.EnvironmentVariable_2m EnvCovar, EDMF_Updrafts.UpdraftVariable_2m UpdCovar,EDMF_Updrafts.UpdraftVariable UpdVar1,
        ...
        #with nogil:
        for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
            EnvCovar.entr_gain[k] = 0.0
            EnvCovar.detr_loss[k] = 0.0
            for i in xrange(self.n_updrafts):
                if self.UpdVar.Area.values[i,k] > self.minimum_area:
                    R_up = self.pressure_plume_spacing*sqrt(self.UpdVar.Area.values[i,k])
                    ...
                    combined_entr = self.Ref.rho0_half[k]*self.UpdVar.Area.values[i,k] * fabs(w_u)*self.entr_sc[i,k]\
                                     + 2.0/(R_up**2.0)*self.Ref.rho0_half[k]*self.UpdVar.Area.values[i,k]*K
                    combined_detr = self.Ref.rho0_half[k]*self.UpdVar.Area.values[i,k] * fabs(w_u)*self.detr_sc[i,k]\
                                     + 2.0/(R_up**2.0)*self.Ref.rho0_half[k]*self.UpdVar.Area.values[i,k]*K

                    UpdCovar.entr_gain[i,k] = combined_entr * (EnvCovar.values[k]   + (updvar1 - envvar1) * (updvar2 - envvar2))
                    UpdCovar.detr_loss[i,k] = combined_detr * UpdCovar.values[i,k]
                    EnvCovar.entr_gain[k]  -= combined_detr * (UpdCovar.values[i,k] + (updvar1 - envvar1) * (updvar2 - envvar2))
                    EnvCovar.detr_loss[k]  -= combined_entr * EnvCovar.values[k]
                else:
                    UpdCovar.entr_gain[i,k] = 0.0
                    UpdCovar.detr_loss[i,k] = 0.0
        return
    ...
    cdef void update_covariance_ED(self, GridMeanVariables GMV, CasesBase Case,TimeStepping TS, VariablePrognostic GmvVar1, VariablePrognostic GmvVar2,
            VariableDiagnostic GmvCovar, EDMF_Environment.EnvironmentVariable_2m EnvCovar, EDMF_Updrafts.UpdraftVariable_2m UpdCovar, EDMF_Environment.EnvironmentVariable  EnvVar1, EDMF_Environment.EnvironmentVariable  EnvVar2,
        ...
        cdef:
            ...
            double  D_env = 0.0
            double Covar_surf, wu_half, K, Kp, R_up
        ...
        self.get_subdomain_covar_from_GMV(self.UpdVar.Area, UpdVar1, UpdVar2, EnvVar1, EnvVar2, UpdCovar, EnvCovar, &GmvVar1.values[0], &GmvVar2.values[0], &GmvCovar.values[0])

        Covar_surf = EnvCovar.values[gw]
        ...
        with nogil:
            for kk in xrange(nz):
                ...
                x[kk] = (self.Ref.rho0_half[k] * ae_old[k] * EnvCovar.values[k] * dti
                         + EnvCovar.press[k] + EnvCovar.buoy[k] + EnvCovar.shear[k] + EnvCovar.entr_gain[k] +  EnvCovar.rain_src[k]) #
        ...
        for kk in xrange(nz):
            k = kk + gw
            if EnvCovar.name == 'thetal_qt_covar':
                EnvCovar.values[k] = fmax(x[kk], - sqrt(self.EnvVar.Hvar.values[k]*self.EnvVar.QTvar.values[k]))
                EnvCovar.values[k] = fmin(x[kk],   sqrt(self.EnvVar.Hvar.values[k]*self.EnvVar.QTvar.values[k]))
            else:
                EnvCovar.values[k] = fmax(x[kk],0.0)
        EnvCovar.set_bcs(self.Gr)
    ...
    cdef void update_upd_covariance_ED(self, GridMeanVariables GMV, CasesBase Case,TimeStepping TS, VariablePrognostic GmvVar1, VariablePrognostic GmvVar2,
        ...
        cdef:
            ...
            double [:] c = np.zeros((nz,),dtype=np.double, order='c')
            double [:] x = np.zeros((nz,),dtype=np.double, order='c')
            double [:] rho_au_K = np.zeros((nzg,),dtype=np.double, order='c')
            double [:] whalf = np.zeros((nzg,),dtype=np.double, order='c')
            double Covar_surf, we_half, D_upd, R_up, K, Kp
        ...
        for i in xrange(self.n_updrafts):
            ...
            for k in xrange(1,nzg-1):
                if EnvCovar.name == 'tke':
                    K = self.upd_KM[i,k]
                    Kp = self.upd_KM[i,k+1]
                else:
                    K = self.upd_KH[i,k]
                    Kp = self.upd_KH[i,k+1]
                rho_au_K[k] = 0.5 * (self.UpdVar.Area.values[i,k]*K + self.UpdVar.Area.values[i,k+1]*Kp)* self.Ref.rho0[k]
            ...
            with nogil:
                for kk in xrange(nz):
                    ...
                    if self.UpdVar.Area.values[i,k] > self.minimum_area:
                        ...
                        a[kk] = (-self.Ref.rho0_half[k-1] * self.UpdVar.Area.values[i,k-1] * whalf[k-1] * dzi - rho_au_K[k-1] * dzi * dzi )
                        b[kk] = (self.Ref.rho0_half[k] * self.UpdVar.Area.values[i,k] * dti + self.Ref.rho0_half[k] * self.UpdVar.Area.values[i,k] * whalf[k] * dzi
                                 + rho_au_K[k] * dzi * dzi + rho_au_K[k-1] * dzi * dzi
    ...
    cpdef compute_upd_tke_buoy(self, GridMeanVariables GMV):
        cdef:
            ...
            double lh, prefactor, cpm
            double qt_dry, th_dry, t_cloudy, qt_cloudy
            double grad_thl_minus=0.0, grad_qt_minus=0.0, grad_thl_plus=0, grad_qt_plus=0
            double [:] ae = np.subtract(np.ones((self.Gr.nzg,),dtype=np.double, order='c'),self.UpdVar.Area.bulkvalues)
                ...
                for k in xrange(gw, self.Gr.nzg-gw):
                    if self.UpdVar.Area.values[i,k] > self.minimum_area:
                        if self.UpdVar.QL.values[i,k] > 0.0:
                            ...
                            th_dry = self.UpdVar.THL.values[i,k]
                            lh = latent_heat(self.UpdVar.T.values[i,k])
                            cpm = cpm_c(self.UpdVar.QT.values[i,k])
                            grad_thl_minus = grad_thl_plus
                            grad_qt_minus = grad_qt_plus
                            ...
                            d_alpha_thetal = prefactor * (1.0 + (eps_vi-1.0) * qt_dry)
                            d_alpha_qt = prefactor * th_dry * (eps_vi-1.0)
