...
cdef class EDMF_PrognosticTKE(ParameterizationBase):
            ...
            for i in xrange(self.n_updrafts):
                for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                    if self.UpdVar.Area.values[i,k] >= self.minimum_area:
                        ...
                        KH_full = ((self.UpdVar.KH.values[i,k]+self.KH.values[k])*0.5+(self.UpdVar.KH.values[i,k+1]+self.KH.values[k+1])*0.5)*0.5
                        self.turb_entr_W[i,k] = 2.0*(self.Ref.rho0[k] * KH_full*(self.UpdVar.W.values[i,k]-self.EnvVar.W.values[k]))/self.pressure_plume_spacing**2.0
                        self.turb_entr_H[i,k] = 2.0*(self.Ref.rho0_half[k] * KH *(self.UpdVar.H.values[i,k]-self.EnvVar.H.values[k]))/self.pressure_plume_spacing**2.0
                        self.turb_entr_QT[i,k] = 2.0*(self.Ref.rho0_half[k] * KH * (self.UpdVar.QT.values[i,k]-self.EnvVar.QT.values[k]))/self.pressure_plume_spacing**2.0
            ...
            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                if self.UpdVar.Area.values[i,k]>self.minimum_area:
                    ...
                    if np.isnan(self.UpdVar.H.diffusion[i,k]):
                        ...
                        print k , self.UpdVar.H.values[i,k+1],  self.UpdVar.H.values[i,k] ,self.UpdVar.H.values[i,k-1]
                        plt.figure()
                        plt.show()
                ...
                for k in range(gw, self.Gr.nzg-gw):
                    ...
                    if self.UpdVar.Area.new[i,k+1] > au_lim:
                        ...
                        if self.UpdVar.Area.values[i,k+1] > 0.0:
                            self.entr_sc[i,k+1] = (((au_lim-self.UpdVar.Area.values[i,k+1])* dti_ - adv -entr_term)/(self.UpdVar.Area.values[i,k+1]  * whalf_kp))
                        else:
                            # this detrainment rate won't affect scalars but would affect velocity
                            self.entr_sc[i,k+1] = (((au_lim-self.UpdVar.Area.values[i,k+1])* dti_ - adv -entr_term)/(au_lim  * whalf_kp))
            ...
            for i in xrange(self.n_updrafts):
                ...
                for k in xrange(gw+1, self.Gr.nzg-gw):
                    ...
                    if self.UpdVar.Area.new[i,k] >= self.minimum_area:
                        ...
                        with gil:
                            if np.isnan(self.UpdVar.H.values[i,k]):
                                print k, self.UpdVar.H.values[i,k], c1, c2 , c3 , c4 , H_entr,  self.turb_entr_H[i,k] , self.UpdVar.H.diffusion[i,k]
                                plt.figure()
                                plt.show()
    ...
    cpdef compute_covariance(self, GridMeanVariables GMV, CasesBase Case, TimeStepping TS):
        ...
        if self.calc_tke:
            ...
            self.compute_covariance_entr(self.EnvVar.TKE, self.UpdVar.TKE, self.UpdVar.W, self.UpdVar.W, self.EnvVar.W, self.EnvVar.W, GMV.TKE)
        ...
        if self.calc_scalar_var:
            self.compute_covariance_entr(self.EnvVar.Hvar, self.UpdVar.Hvar, self.UpdVar.H, self.UpdVar.H, self.EnvVar.H, self.EnvVar.H, GMV.Hvar)
            self.compute_covariance_entr(self.EnvVar.QTvar, self.UpdVar.QTvar, self.UpdVar.QT, self.UpdVar.QT, self.EnvVar.QT, self.EnvVar.QT, GMV.QTvar)
            self.compute_covariance_entr(self.EnvVar.HQTcov, self.UpdVar.HQTcov, self.UpdVar.H, self.UpdVar.QT, self.EnvVar.H, self.EnvVar.QT, GMV.HQTcov)
    ...
    #             UpdCovar.entr[i,k] += self.Ref.rho0_half[k]*self.UpdVar.Area.values[i,k] * fabs(w_u) * self.entr_sc[i,k] * \
    ...
    cdef void compute_covariance_entr(self, EDMF_Environment.EnvironmentVariable_2m EnvCovar, EDMF_Updrafts.UpdraftVariable_2m UpdCovar, EDMF_Updrafts.UpdraftVariable UpdVar1,
        ...
        for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
            ...
            for i in xrange(self.n_updrafts):
                ...
                UpdCovar.entr[i,k] = self.Ref.rho0_half[k]*self.UpdVar.Area.values[i,k] * fabs(w_u) * self.entr_sc[i,k] * GmvCovar.values[k]
                EnvCovar.entr[k] -= UpdCovar.entr[i,k]
        ...
        with nogil:
            for kk in xrange(nz):
                ...
                x[kk] = (self.Ref.rho0_half[k] * ae_old[k] * Covar.values[k] * dti
                         + Covar.press[k] + Covar.buoy[k] + Covar.shear[k] + Covar.entr[k]
                         + Covar.turb_entr[k] + Covar.massflux_entr[k] +  Covar.rain_src[k]) #
...
## ---------- updraft coavriances
    ...
    cdef void update_upd_covariance_ED(self, GridMeanVariables GMV, CasesBase Case,TimeStepping TS, VariablePrognostic GmvVar1, VariablePrognostic GmvVar2,
        ...
        return
    # TODO assuming one updraft for now - the alternative is to calcualte each upd from all other upd and env and gmv
    # cdef get_upd_covar_from_GMV(self, EDMF_Updrafts.UpdraftVariable au,
    #                             EDMF_Updrafts.UpdraftVariable phi_u, EDMF_Updrafts.UpdraftVariable psi_u,
    #                             EDMF_Environment.EnvironmentVariable phi_e, EDMF_Environment.EnvironmentVariable psi_e,
    #                             EDMF_Updrafts.UpdraftVariable_2m covar_u,EDMF_Environment.EnvironmentVariable_2m covar_e,
