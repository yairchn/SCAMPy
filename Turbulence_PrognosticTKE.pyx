...
cdef class EDMF_PrognosticTKE(ParameterizationBase):
    ...
    cpdef compute_entrainment_detrainment(self, GridMeanVariables GMV, CasesBase Case, TimeStepping TS):
        ...
        # find k_top as the last z index where a>0
        ...
        for i in xrange(self.n_updrafts):
            ...
            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                # in z levels where LES has an updraft 
                if k <= ktop:
                    if (self.UpdVar.Area.values[i,k]>0.0) and (self.Gr.z_half[k]>20.0):
                ...
                # in z levels above the LES updraft - uhse the closure
                else:
                    if self.UpdVar.Area.values[i,k]>0.0:
    ...
    cpdef compute_nh_pressure(self, TimeStepping TS):
        ...
        z_les = np.multiply(data.groups['reference'].variables['zp_half'],1.0)
        # find k_top as the last z index where a_upd>0
        k_ztop = int(np.max(np.where(z < a_upd>0)[0]))
        dapdz_upd_ = np.multiply(np.nanmean(data.groups['profiles'].variables['updraft_ddz_p_alpha'][180:-1],axis=0),1.0)
        dapdz_upd = np.interp(self.Gr.z_half[self.Gr.gw:self.Gr.nzg-self.Gr.gw], z_les, dapdz_upd_)

        for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
            # ADD IF A_UPD>0 else: self.nh_pressure[i,k] = 0.0
            for i in xrange(self.n_updrafts):
                if k <= k_ztop:
                    if (self.UpdVar.Area.values[i,k]>0.0) and (self.Gr.z_half[k]>20.0):
