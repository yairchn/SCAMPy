...
cdef class EDMF_PrognosticTKE(ParameterizationBase):
    ...
    def __init__(self, namelist, paramlist, Grid Gr, ReferenceState Ref):
        ...
        self.EnvVar = EDMF_Environment.EnvironmentVariables(namelist,paramlist,Gr)
        ...
        print 189
        self.a_numerical = np.zeros((self.n_updrafts, Gr.nzg,),dtype=np.double,order='c')

        # Near-surface BC of updraft area fraction
        self.area_upd_surface_bc= np.zeros((self.n_updrafts,),dtype=np.double, order='c')
        self.w_upd_surface_bc= np.zeros((self.n_updrafts,),dtype=np.double, order='c')
        self.h_upd_surface_flux= np.zeros((self.n_updrafts,),dtype=np.double, order='c')
        self.qt_upd_surface_flux= np.zeros((self.n_updrafts,),dtype=np.double, order='c')
        self.h_env_surface_flux= 0.0
        self.qt_env_surface_flux= 0.0
        self.area_env_surface_bc= 0.0
        self.w_env_surface_bc= 0.0
    ...
    cpdef initialize_io(self, NetCDFIO_Stats Stats):
        ...
        Stats.add_profile('upd_mixing_length')
        ...
        if self.calc_tke:
            ...
            Stats.add_profile('updraft_w_diffusion')
            Stats.add_profile('updraft_H_diffusion')
            Stats.add_profile('updraft_QT_diffusion')

            Stats.add_profile('upd_tke_buoy')
            Stats.add_profile('upd_tke_dissipation')
            Stats.add_profile('upd_tke_entr')
            Stats.add_profile('upd_tke_turb_entr')
            ...
            Stats.add_profile('env_tke_entr')
            Stats.add_profile('env_tke_turb_entr')
        ...
        if self.calc_scalar_var:
            ...
            Stats.add_profile('env_QTvar_dissipation')
            Stats.add_profile('env_HQTcov_dissipation')
            Stats.add_profile('env_Hvar_entr')
            Stats.add_profile('env_QTvar_entr')
            Stats.add_profile('env_Hvar_turb_entr')
            Stats.add_profile('env_QTvar_turb_entr')
            Stats.add_profile('env_HQTcov_entr')
            Stats.add_profile('env_HQTcov_turb_entr')
            Stats.add_profile('env_Hvar_shear')
            Stats.add_profile('env_QTvar_shear')
            Stats.add_profile('env_HQTcov_shear')
            ...
            Stats.add_profile('upd_HQTcov_dissipation')
            Stats.add_profile('upd_Hvar_entr')
            Stats.add_profile('upd_QTvar_entr')
            Stats.add_profile('upd_Hvar_turb_entr')
            Stats.add_profile('upd_QTvar_turb_entr')
            Stats.add_profile('upd_HQTcov_entr')
            Stats.add_profile('upd_HQTcov_turb_entr')
            Stats.add_profile('upd_Hvar_shear')
            Stats.add_profile('upd_QTvar_shear')
            Stats.add_profile('upd_HQTcov_shear')
    ...
    cpdef io(self, NetCDFIO_Stats Stats):
        ...
        Stats.write_profile('upd_mixing_length', self.upd_mixing_length[0,kmin:kmax])
        ...
        if self.calc_tke:
            ...
            Stats.write_profile('env_tke_entr', self.EnvVar.TKE.entr[kmin:kmax])
            ...
            Stats.write_profile('upd_tke_entr', self.UpdVar.TKE.entr[0,kmin:kmax])
        ...
        if self.calc_scalar_var:
            ...
            Stats.write_profile('env_Hvar_entr', self.EnvVar.Hvar.entr[kmin:kmax])
            Stats.write_profile('env_QTvar_entr', self.EnvVar.QTvar.entr[kmin:kmax])
            Stats.write_profile('env_HQTcov_entr', self.EnvVar.HQTcov.entr[kmin:kmax])
            ...
            Stats.write_profile('upd_Hvar_entr', self.UpdVar.Hvar.entr[0,kmin:kmax])
            Stats.write_profile('upd_QTvar_entr', self.UpdVar.QTvar.entr[0,kmin:kmax])
            Stats.write_profile('upd_HQTcov_entr', self.UpdVar.HQTcov.entr[0,kmin:kmax])
        ...
        if TS.nstep == 0:
            ...
            with nogil:
                for k in xrange(self.Gr.nzg):
                    ...
                    for i in xrange(self.n_updrafts):
                        self.UpdVar.TKE.values[i,k] = GMV.TKE.values[k]
                        self.UpdVar.Hvar.values[i,k] = GMV.Hvar.values[k]
                        self.UpdVar.QTvar.values[i,k] = GMV.QTvar.values[k]
                        self.UpdVar.HQTcov.values[i,k] = GMV.HQTcov.values[k]
    ...
    #     self.compute_w_pressure_term(Case)
    #     self.compute_updraft_diffusion(Case)
    ...
    #     with nogil:
    #         for i in xrange(self.n_updrafts):
    #             self.UpdVar.H.values[i,gw] = self.h_surface_upd_flux[i]
    #             self.UpdVar.QT.values[i,gw] = self.qt_surface_upd_flux[i]
    ...
    #                                                     (1.0-(self.entr_sc[i,k])*dz)/w_mid/self.Ref.rho0_half[k])
    ...
    cpdef update_buoyancy(self,GridMeanVariables GMV):
        ...
        self.UpdVar.Area.bulkvalues = np.sum(self.UpdVar.Area.values,axis=0)

        with nogil:
            for k in xrange(self.Gr.nzg):
                sa = eos(self.UpdThermo.t_to_prog_fp,self.UpdThermo.prog_to_t_fp,
                             self.Ref.p0_half[k], self.EnvVar.QT.values[k], self.EnvVar.H.values[k])

                self.EnvVar.QL.values[k] = sa.ql
                self.EnvVar.T.values[k] = sa.T

                qv = self.EnvVar.QT.values[k] - self.EnvVar.QL.values[k]
                alpha = alpha_c(self.Ref.p0_half[k], self.EnvVar.T.values[k], self.EnvVar.QT.values[k], qv)
                self.EnvVar.B.values[k] = buoyancy_c(self.Ref.alpha0_half[k], alpha)
                GMV.B.values[k] = (1.0 - self.UpdVar.Area.bulkvalues[k]) * self.EnvVar.B.values[k]
                for i in xrange(self.n_updrafts):
                    sa = eos(self.UpdThermo.t_to_prog_fp,self.UpdThermo.prog_to_t_fp,
                             self.Ref.p0_half[k], self.UpdVar.QT.values[i,k], self.UpdVar.H.values[i,k])

                    self.UpdVar.QL.values[i,k] = sa.ql
                    self.UpdVar.T.values[i,k] = sa.T

                    qv = self.UpdVar.QT.values[i,k] - self.UpdVar.QL.values[i,k]
                    alpha = alpha_c(self.Ref.p0_half[k], self.UpdVar.T.values[i,k], self.UpdVar.QT.values[i,k], qv)
                    self.UpdVar.B.values[i,k] = buoyancy_c(self.Ref.alpha0_half[k], alpha)
        ...
        if (self.mixing_scheme == 'sbl'):
            ...
            for k in xrange(gw, self.Gr.nzg-gw):
                ...
                if obukhov_length < 0.0: #unstable
                    l1 = vkb * z_ / self.tke_ed_coeff / 3.75 # * ( (1.0 - 100.0 * z_/obukhov_length)**0.2 )
                elif obukhov_length > 0.0: #stable
                    l1 = vkb * z_ / self.tke_ed_coeff / 3.75 #/  (1. + 2.7 *z_/obukhov_length)
                else:
                ...
                l2 = fmin(sqrt(fmax(0.35*self.EnvVar.TKE.values[k],0.0))/N, 1000.0)

                l[0]=l1; l[1]=l2; l[2]=l3; l[3]=l4; l[4]=l5 ; l[5]=1.0e5
            ...
            for k in xrange(gw, self.Gr.nzg-gw):
                ...
                for i in xrange(self.n_updrafts):
                    if self.UpdVar.Area.values[i,k]>self.minimum_area:
                        ...
                        if (sqrt(shear2)< m_eps or 1.0 - ri_thl/fmax(prandtl, m_eps) - ri_qt/fmax(prandtl, m_eps) < m_eps):
                            l3 = 1.0e6
                        l4 = sqrt(0.5*self.tke_diss_coeff/fmax(self.tke_ed_coeff, m_eps)) * sqrt(fmax(self.UpdVar.Hvar.values[i,k],0.0))/fmax(sqrt(grad_thl), m_eps)
                        if sqrt(grad_thl)< m_eps:
                            l4 = 1.0e6
                        l5 = sqrt(0.5*self.tke_diss_coeff/fmax(self.tke_ed_coeff, m_eps)) * sqrt(fmax(self.UpdVar.QTvar.values[i,k],0.0))/fmax(sqrt(grad_qt), m_eps)
                        ...
                        try:
                            ...
                            N = 1.0e6
                        l2 = fmin(sqrt(fmax(0.8*self.UpdVar.TKE.values[i,k],0.0))/N, 1.0e6)
        ...
        # Tan et al. (2018)
            ...
            with nogil:
                ...
                for k in xrange(gw, self.Gr.nzg-gw):
                    for i in xrange(self.n_updrafts):
                        if self.UpdVar.Area.values[i,k]>self.minimum_area:
                            l1 = tau * sqrt(fmax(self.UpdVar.TKE.values[i,k],0.0))
                            z_ = self.Gr.z_half[k]
                            if obukhov_length < 0.0: #unstable
                                l2 = vkb * z_ * ( (1.0 - 100.0 * z_/obukhov_length)**0.2 )
                            elif obukhov_length > 0.0: #stable
    ...
    cpdef set_subdomain_bc(self, GridMeanVariables GMV, CasesBase Case):
        ...
        # with nogil:
        surface_scalar_coeff = percentile_bounds_mean_norm(0.0,1.0-self.surface_area , 1000)
        self.area_env_surface_bc = 1.0-self.surface_area
        self.w_env_surface_bc = 0.0
        self.h_env_surface_flux = Case.Sur.rho_hflux*(1.0 - (surface_scalar_coeff * sqrt(h_var))/GMV.H.values[gw])
        self.qt_env_surface_flux = Case.Sur.rho_qtflux*(1.0 - (surface_scalar_coeff * sqrt(qt_var))/GMV.QT.values[gw])

        for i in xrange(self.n_updrafts):
            surface_scalar_coeff = percentile_bounds_mean_norm(1.0-self.surface_area+i*a_,
                                                                   1.0-self.surface_area + (i+1)*a_ , 1000)

            self.area_upd_surface_bc[i] = self.surface_area/self.n_updrafts
            self.w_upd_surface_bc[i] = 0.0
            self.h_upd_surface_flux[i] = Case.Sur.rho_hflux*(1.0+(surface_scalar_coeff * sqrt(h_var))/GMV.H.values[gw])
            self.qt_upd_surface_flux[i] = Case.Sur.rho_qtflux*(1.0+(surface_scalar_coeff * sqrt(qt_var))/GMV.QT.values[gw])

            self.h_env_surface_flux -= self.h_upd_surface_flux[i]
            self.qt_env_surface_flux -= self.qt_upd_surface_flux[i]
    ...
    cpdef decompose_environment(self, GridMeanVariables GMV, whichvals):
        ...
        if whichvals == 'values':
            ...
            if self.calc_tke:
                self.get_GMV_CoVar(self.UpdVar.Area,self.UpdVar.W, self.UpdVar.W, self.EnvVar.W, self.EnvVar.W, self.EnvVar.TKE, self.UpdVar.TKE, &GMV.W.values[0],&GMV.W.values[0], &GMV.TKE.values[0])
            if self.calc_scalar_var:
                self.get_GMV_CoVar(self.UpdVar.Area,self.UpdVar.H, self.UpdVar.H, self.EnvVar.H, self.EnvVar.H, self.EnvVar.Hvar, self.UpdVar.Hvar, &GMV.H.values[0],&GMV.H.values[0], &GMV.Hvar.values[0])
                self.get_GMV_CoVar(self.UpdVar.Area,self.UpdVar.QT,self.UpdVar.QT,self.EnvVar.QT,self.EnvVar.QT, self.EnvVar.QTvar, self.UpdVar.QTvar, &GMV.QT.values[0],&GMV.QT.values[0], &GMV.QTvar.values[0])
                self.get_GMV_CoVar(self.UpdVar.Area,self.UpdVar.H, self.UpdVar.QT,self.EnvVar.H, self.EnvVar.QT, self.EnvVar.HQTcov, self.UpdVar.HQTcov, &GMV.H.values[0],&GMV.QT.values[0], &GMV.HQTcov.values[0])
    ...
    cdef get_GMV_CoVar(self, EDMF_Updrafts.UpdraftVariable au,
        ...
        cdef:
            ...
            double phi_diff, psi_diff, phi_diff_env, psi_diff_env, phi_upd

        for k in xrange(self.Gr.nzg):
            # EnvVar contribution
            gmv_covar[k] = ae[k]*covar_e.values[k]
            for i in xrange(self.n_updrafts):
                # UpdVar contribution
                gmv_covar[k] += au.values[i,k]*covar_u.values[i,k]
                if covar_u.name == 'tke':
                    phi_upd = interp2pt(phi_u.values[i,k-1] ,phi_u.values[i,k])
                ...
                gmv_covar[k]  += tke_factor *au.values[i,k] * ae[k] * phi_diff_env * psi_diff_env
    ...
    cdef get_env_upd_covar_from_GMV(self, EDMF_Updrafts.UpdraftVariable au,
                                EDMF_Updrafts.UpdraftVariable phi_u, EDMF_Updrafts.UpdraftVariable psi_u,
                                EDMF_Environment.EnvironmentVariable phi_e, EDMF_Environment.EnvironmentVariable psi_e,
        ...
        cdef:
            Py_ssize_t i,k
            double [:] ae = np.subtract(np.ones((self.Gr.nzg,),dtype=np.double, order='c'),au.bulkvalues)
            double phi_diff, psi_diff, phi_diff_env, psi_diff_env, phi_i
            double tke_factor = 1.0
            double mf, updvar
        ...
        for i in xrange(self.n_updrafts):
            if au.values[i,k] > self.minimum_area:
                if covar_u.name == 'tke':
                    phi_i = interp2pt(phi_u.values[i,k-1] ,phi_u.values[i,k])
                    phi_diff_env = interp2pt(phi_u.values[i,k-1] - phi_e.values[k-1],phi_u.values[i,k] - phi_e.values[k])
                    psi_diff_env = interp2pt(psi_u.values[i,k-1] - psi_e.values[k-1],psi_u.values[i,k] - psi_e.values[k])
                    tke_factor = 0.5
                else:
                    phi_i = phi_u.values[i,k]
                    phi_diff_env = phi_u.values[i,k] - phi_e.values[k]
                    psi_diff_env = psi_u.values[i,k] - psi_e.values[k]
                    tke_factor = 1.0
    ...
    #         Py_ssize_t i,k
    #         double [:] ae = np.subtract(np.ones((self.Gr.nzg,),dtype=np.double, order='c'),au.bulkvalues)
    #         double phi_diff, psi_diff, phi_i, phi_diff_env, psi_diff_env, updvar
    #         double tke_factor = 1.0
    #         double mf = 0.0
    #         double upd = 0.0
    #     if covar_e.name == 'tke':
    ...
    #             updvar = 0.0
    #             for i in xrange(self.n_updrafts):
    #                 updvar += au.values[i,k]*covar_u.values[i,k]
    #                 if covar_u.name == 'tke':
    #                     phi_i = interp2pt(phi_u.values[i,k-1] ,phi_u.values[i,k])
    #                     phi_diff_env = interp2pt(phi_u.values[i,k-1] - phi_e.values[k-1],phi_u.values[i,k] - phi_e.values[k])
    #                     psi_diff_env = interp2pt(psi_u.values[i,k-1] - psi_e.values[k-1],psi_u.values[i,k] - psi_e.values[k])
    ...
    #             #covar_e.values[k] = (gmv_covar[k] - updvar - tke_factor * mf)/ae[k]
    #             covar_e.values[k] = (gmv_covar[k] - tke_factor * mf)/ae[k]/(self.n_updrafts+1.0)
    #         else:
    ...
    #                     if j != i:
    #                         updvar += au.values[j,k]*covar_u.values[j,k]
    #                 #covar_u.values[i,k] = (gmv_covar[k] - updvar - ae[k]*covar_e.values[k] - tke_factor * mf)/au.values[i,k]
    #                 covar_u.values[i,k] = (gmv_covar[k] - ae[k]*covar_e.values[k] - tke_factor * mf)/au.values[i,k]/(self.n_updrafts+1.0)
    #             else:
    #                 covar_u.values[i,k] = 0.0
    #     return
    ...
    #TODO central differneces
    cpdef compute_turbulent_diffusion(self, CasesBase Case):
        cdef:
            double a_k, B_k, press_buoy, press_drag
            double dw = 0.0
            double dH = 0.0
        ...
        self.EnvVar.W.diffusion[self.Gr.gw-1] = 0.0
        for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):

            rho_au_Km_low = self.Ref.rho0_half[k] * self.EnvVar.Area.values[k]*self.KH.values[k]
            rho_au_Km_high = self.Ref.rho0_half[k+1]* self.EnvVar.Area.values[k+1]*self.KH.values[k+1]
            dw_low = (self.EnvVar.W.values[k] - self.EnvVar.W.values[k-1]) * dzi
            dw_high = (self.EnvVar.W.values[k+1] - self.EnvVar.W.values[k]) * dzi

            rho_au_Km_full_low = 0.5 * (self.Ref.rho0_half[k-1]* self.EnvVar.Area.values[k-1]*self.KH.values[k-1]+
                                   self.Ref.rho0_half[k]* self.EnvVar.Area.values[k]*self.KH.values[k])
            rho_au_Km_full_high = 0.5 * (self.Ref.rho0_half[k]* self.EnvVar.Area.values[k]*self.KH.values[k]+
                                   self.Ref.rho0_half[k+1]* self.EnvVar.Area.values[k+1]*self.KH.values[k+1])
            dH_low = 0.0*(self.EnvVar.H.values[k] -  self.EnvVar.H.values[k-1])*dzi
            dQT_low = 0.0*(self.EnvVar.QT.values[k] -  self.EnvVar.QT.values[k-1])*dzi
            dH_high = 0.0*(self.EnvVar.H.values[k+1] - self.EnvVar.H.values[k])*dzi
            dQT_high = 0.0*(self.EnvVar.QT.values[k+1] - self.EnvVar.QT.values[k])*dzi

            self.EnvVar.W.diffusion[k] = (rho_au_Km_high*dw_high - rho_au_Km_low*dw_low) *dzi
            self.EnvVar.H.diffusion[k] = (rho_au_Km_full_high*dH_high - rho_au_Km_full_low*dH_low) *dzi
            self.EnvVar.QT.diffusion[k] = (rho_au_Km_full_high*dQT_high - rho_au_Km_full_low*dQT_low) *dzi
    ...
    cpdef solve_subdomains(self, GridMeanVariables GMV, CasesBase Case, TimeStepping TS):
        cdef:
            Py_ssize_t i, k
            Py_ssize_t gw = self.Gr.gw
            double dzi = self.Gr.dzi
        ...
        for i in xrange(self.n_updrafts):
            ...
            self.UpdVar.Area.values[i,gw] = self.area_upd_surface_bc[i]
            self.UpdVar.Area.new[i,gw] = self.area_upd_surface_bc[i]
            self.a_numerical[i,gw] =  self.area_upd_surface_bc[i]
            ...
            #                                                        i, gw)
            # surface to gw
            a_full = interp2pt(self.a_numerical[i,gw]  , self.a_numerical[i,gw+1] )
            if a_full >= self.minimum_area:
                self.updraft_integration(self.UpdVar.Area, self.UpdVar.W, gw, i, GMV.W.values[gw],dzi*2.0)

            if self.a_numerical[i,gw] >= self.minimum_area:
                self.updraft_integration(self.UpdVar.Area, self.UpdVar.H, gw, i, GMV.H.values[gw],dzi*2.0)
                self.updraft_integration(self.UpdVar.Area, self.UpdVar.QT, gw, i, GMV.QT.values[gw],dzi*2.0)
                print 1579, self.UpdVar.H.new[i,gw], self.UpdVar.QT.new[i,gw]
                plt.figure()
                plt.show()

            for k in range(gw+1, self.Gr.nzg-gw):
                self.updraft_integration(self.UpdVar.Area, self.UpdVar.Area, k, i, 1.0, dzi)
                if interp2pt(self.a_numerical[i,k],self.a_numerical[i,k+1]) >= self.minimum_area:
                    self.updraft_integration(self.UpdVar.Area, self.UpdVar.W, k, i, GMV.W.values[k],dzi)
                else:
                    self.UpdVar.W.new[i,k] = 0.0
                    self.UpdVar.Area.new[i,k+1] = 0.0
                    self.a_numerical[i,k+1] = 0.0
                    self.updraft_pressure_sink[i,k] = 0.0 # keep this in mind if we modify updraft top treatment!

                if self.a_numerical[i,k] >= self.minimum_area:
                    self.updraft_integration(self.UpdVar.Area, self.UpdVar.H, k, i, GMV.H.values[k], dzi)
                    self.updraft_integration(self.UpdVar.Area, self.UpdVar.QT, k, i, GMV.QT.values[k], dzi)
                else:
                    self.UpdVar.H.new[i,k] = GMV.H.values[k]
                    self.UpdVar.QT.new[i,k] = GMV.QT.values[k]

                print 1599, self.UpdVar.QT.new[i,k], self.UpdVar.H.new[i,k]
                sa = eos(self.UpdThermo.t_to_prog_fp,self.UpdThermo.prog_to_t_fp, self.Ref.p0_half[k],
        ...
        self.EnvVar.Area.values[gw] = 1.0-self.area_upd_surface_bc[i]
        self.EnvVar.Area.new[gw] = 1.0-self.area_upd_surface_bc[i]

        sa = eos(self.EnvThermo.t_to_prog_fp,self.EnvThermo.prog_to_t_fp,
                self.Ref.p0_half[gw], self.EnvVar.QT.new[gw], self.EnvVar.H.new[gw])

        self.EnvVar.QL.new[gw] = sa.ql
        self.EnvVar.T.new[gw] = sa.T

        #self.UpdMicro.compute_update_combined_local_thetal(self.Ref.p0_c[gw], self.UpdVar.T.new[i,gw],
        #                                                       &self.UpdVar.QT.new[i,gw], &self.UpdVar.QL.new[i,gw],
        #                                                       &self.UpdVar.QR.new[i,gw], &self.UpdVar.H.new[i,gw],
        #                                                       i, gw)
        self.environment_integration(self.EnvVar.Area,self.EnvVar.W, gw, GMV.W.values[gw], dzi)
        self.environment_integration(self.EnvVar.Area, self.EnvVar.H, gw, GMV.H.values[gw], dzi)
        self.environment_integration(self.EnvVar.Area, self.EnvVar.QT, gw, GMV.QT.values[gw], dzi)
        self.EnvVar.Area.new = np.subtract(np.ones((self.Gr.nzg,),dtype=np.double, order='c'),self.UpdVar.Area.bulkvalues)
        for k in range(gw+1, self.Gr.nzg-gw):
            if self.EnvVar.Area.new[k] <1.0:
                self.environment_integration(self.EnvVar.Area, self.EnvVar.W, k, GMV.W.values[k],dzi)
                self.environment_integration(self.EnvVar.Area, self.EnvVar.H, k, GMV.H.values[k], dzi)
                self.environment_integration(self.EnvVar.Area, self.EnvVar.QT, k, GMV.QT.values[k], dzi)
            else:
                self.EnvVar.W.new[k] = 0.0
                self.EnvVar.H.new[k] = GMV.H.values[k]
                self.EnvVar.QT.new[k] = GMV.QT.values[k]

            print 1641, self.EnvVar.QT.new[k], self.EnvVar.H.new[k]
            sa = eos(self.EnvThermo.t_to_prog_fp,self.EnvThermo.prog_to_t_fp, self.Ref.p0_half[k],
                         self.EnvVar.QT.new[k], self.EnvVar.H.new[k])

            self.EnvVar.QL.new[k] = sa.ql
            self.EnvVar.T.new[k] = sa.T

            # if self.use_local_micro:
            #     self.UpdMicro.compute_update_combined_local_thetal(self.Ref.p0_c[k], self.EnvVar.T.new[k],
            #                                                        &self.EnvVar.QT.new[k], &self.EnvVar.QL.new[k],
            #                                                        &self.EnvVar.QR.new[k], &self.EnvVar.H.new[k],
            #                                                        k)
        self.UpdVar.H.set_bcs(self.Gr)
        self.UpdVar.QT.set_bcs(self.Gr)
        self.UpdVar.QR.set_bcs(self.Gr)
        ...
        self.EnvVar.QR.set_bcs(self.Gr)
        return
    ...
    cpdef updraft_integration(self, EDMF_Updrafts.UpdraftVariable area, EDMF_Updrafts.UpdraftVariable var, int k, int i, double gmv_var, double dzi):
        cdef:
            ...
            double flux_divergence, rhs_, exch
            double var_kp ,var_k, var_km, area_new, sgn_w
            double au_lim = self.area_upd_surface_bc[i] * self.max_area_factor

        if var.name == 'w':
            buoy = self.Ref.rho0_half[k] * area.values[i,k] * interp2pt(self.UpdVar.B.values[i,k], self.UpdVar.B.values[i,k+1])
            area_new = interp2pt(area.new[i,k], area.new[i,k+1])
            area_values = interp2pt(area.values[i,k], area.values[i,k+1])
            exch = self.turb_entr_W[i,k]
            rhs_ = exch + self.w_press_term[i,k] + buoy +  self.UpdVar.W.diffusion[i,k]
            self.updraft_pressure_sink[i,k] = self.w_press_term[i,k]

            flux_top = self.Ref.rho0_half[k] * area.values[i,k] * self.UpdVar.W.values[i,k]**2.0
            if k==self.Gr.gw:
                flux_bottom = 0.0
            else:
                flux_bottom = self.Ref.rho0_half[k-1] * area.values[i,k-1] * self.UpdVar.W.values[i,k-1]**2.0


        elif var.name == 'thetal':
            area_new = area.new[i,k]
            area_values = area.values[i,k]
            exch = self.Ref.alpha0_half[k]* area_values  * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k])*self.entr_sc[i,k] \
                    * gmv_var + self.turb_entr_H[i,k]
            rhs_ = exch +  self.UpdVar.H.diffusion[i,k] # microphysics

            flux_top = self.Ref.rho0_half[k] * area.values[i,k] * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k]) * var.values[i,k]
            if k==self.Gr.gw:
                flux_bottom = self.h_upd_surface_flux[i]
            else:
                flux_bottom = self.Ref.rho0_half[k-1] * area.values[i,k-1] * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k-2]) * var.values[i,k-1]
            print  1701, 'flux_bottom_h',flux_bottom, area_new, self.h_upd_surface_flux[i]

        elif var.name == 'qt':
            area_new = area.new[i,k]
            area_values = area.values[i,k]
            exch = self.Ref.alpha0_half[k]* area_values  * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k])*self.entr_sc[i,k] \
                    * gmv_var + self.turb_entr_QT[i,k]
            rhs_ = exch +  self.UpdVar.H.diffusion[i,k] # microphysics
            flux_top = self.Ref.rho0_half[k] * area.values[i,k] * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k]) * var.values[i,k]
            if k==self.Gr.gw:
                flux_bottom = self.qt_upd_surface_flux[i]
            else:
                flux_bottom = self.Ref.rho0_half[k-1] * area.values[i,k-1] * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k-2]) * var.values[i,k-1]
            print 1713, 'flux_bottom_qt',flux_bottom, area_new, self.qt_upd_surface_flux[i]

        elif var.name =='area_fraction':
            area_new = 1.0
            area_values = area.values[i,k]
            exch = self.Ref.alpha0_half[k]* area_values  * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k])*self.entr_sc[i,k]
            rhs_ = exch
            flux_top = self.Ref.rho0_half[k] * area.values[i,k] * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k])
            flux_bottom = self.Ref.rho0_half[k-1] * area.values[i,k-1] * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k-2])

        #with nogil:
        flux_divergence = -(flux_top - flux_bottom) * dzi
        var.new[i,k] = (self.Ref.rho0_half[k] * var.values[i,k]* area.values[i,k]  + (flux_divergence + rhs_)*dt_)\
                      /(self.Ref.rho0_half[k] * area_new)
        #print 1727, k, var.name, flux_divergence, var.new[i,k], rhs_, area_new, dzi, dt_
        #print self.Ref.rho0_half[k] * area_new, (self.Ref.rho0_half[k] * var.values[i,k] + (flux_divergence + rhs_)*dt_)
        #plt.figure()
        #plt.show()

        if var.name =='area_fraction':
            self.a_numerical[i,k] = var.new[i,k]
            ...
            if var.new[i,k] > au_lim:
                ...
                if var.values[i,k] > 0.0:
                    self.entr_sc[i,k] = (((au_lim-area.values[i,k])* dti_ - flux_divergence)/(-area.new[i,k]  \
                        * interp2pt(self.UpdVar.W.values[i,k], self.UpdVar.W.values[i,k-1])))
                else:
                    self.entr_sc[i,k] = (((au_lim-area.values[i,k])* dti_ - flux_divergence)/(-au_lim  \
                        * interp2pt(self.UpdVar.W.values[i,k], self.UpdVar.W.values[i,k-1])))

        #clipping and avoiding downward integration
        if var.name == 'w':
    ...
    cpdef environment_integration(self, EDMF_Environment.EnvironmentVariable area, EDMF_Environment.EnvironmentVariable var, int k,  double gmv_var, double dzi):
        cdef:
        ...
        if var.name == 'w':
            buoy = self.Ref.rho0_half[k] * area.values[k] * interp2pt(self.EnvVar.B.values[k], self.EnvVar.B.values[k+1])
            area_new = interp2pt(area.new[k], area.new[k+1])
            area_values = interp2pt(area.values[k], area.values[k+1])
            press = 0.0
            exch = 0.0
            for i in xrange(self.n_updrafts):
                press -= self.w_press_term[i,k]
                exch -= self.turb_entr_W[i,k] 
            rhs_ = exch + press + buoy +  self.EnvVar.W.diffusion[k] 
            flux_top = self.Ref.rho0_half[k] * area.values[k] * self.EnvVar.W.values[k]**2.0
            if k==self.Gr.gw:
                flux_bottom = 0.0
            else:
                flux_bottom = self.Ref.rho0_half[k] * area.values[k] * self.EnvVar.W.values[k]**2.0


        elif var.name == 'thetal':
            area_new = area.new[k]
            area_values = area.values[k]
            exch = 0.0
            for i in xrange(self.n_updrafts):
                exch -= self.Ref.alpha0_half[k]* self.UpdVar.Area.values[i,k]  * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k])*self.entr_sc[i,k] \
                        * gmv_var + self.turb_entr_H[i,k]
            rhs_ = exch +  self.EnvVar.H.diffusion[k] # microphysics

            flux_top = self.Ref.rho0_half[k] * area.values[k] * interp2pt(self.EnvVar.W.values[k-1],self.EnvVar.W.values[k]) * var.values[k]
            if k==self.Gr.gw:
                flux_bottom = self.h_env_surface_flux
            else:
                flux_bottom = self.Ref.rho0_half[k-1] * area.values[k-1] * interp2pt(self.EnvVar.W.values[k-1],self.EnvVar.W.values[k-2]) * var.values[k-1]
            ...
            flux_top = self.Ref.rho0_half[k] * area.values[k] * interp2pt(self.EnvVar.W.values[k-1],self.EnvVar.W.values[k]) * var.values[k]
            if k==self.Gr.gw:
                flux_bottom = self.qt_env_surface_flux
            else:
                flux_bottom = self.Ref.rho0_half[k-1] * area.values[k-1] * interp2pt(self.EnvVar.W.values[k-1],self.EnvVar.W.values[k-2]) * var.values[k-1]
        elif var.name =='area_fraction':
            area_new = 1.0
            ...
            for i in xrange(self.n_updrafts):
                exch -= self.Ref.alpha0_half[k] * self.UpdVar.Area.values[i,k]  * interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k])*self.entr_sc[i,k]
            rhs_ = exch
            flux_top = self.Ref.rho0_half[k] * area.values[k] * interp2pt(self.EnvVar.W.values[k-1],self.EnvVar.W.values[k])
            flux_bottom = self.Ref.rho0_half[k-1] * area.values[k-1] *interp2pt(self.EnvVar.W.values[k-1],self.EnvVar.W.values[k-2])

        #with nogil:
        #print k, var.name,  var.new[k], flux_divergence, rhs_, area_new
        flux_divergence = -(flux_top - flux_bottom) * dzi
        var.new[k] = (self.Ref.rho0_half[k] * var.values[k] * area.values[k] + (flux_divergence + rhs_)*dti_ )\
                      /(self.Ref.rho0_half[k] * area_new )

        # clipping and avoiding upward integration
        #if var.name == 'w':
        #    if self.EnvVar.W.new[k] > 0.0:
        #        print 'clipping env',k, self.EnvVar.W.new[k], self.EnvVar.Area.new[k]
        #        self.EnvVar.W.new[k] = 0.0
        #        self.EnvVar.Area.new[k+1] = 1.0

        return

    cpdef update_GMV(self, GridMeanVariables GMV):
        ...
        for k in range(self.Gr.nzg):
            GMV.W.values[k] = self.EnvVar.W.values[k]*ae[k]
            GMV.H.values[k] = self.EnvVar.H.values[k]*ae[k]
            GMV.QT.values[k] = self.EnvVar.QT.values[k]*ae[k]
            GMV.T.values[k] = self.EnvVar.T.values[k]*ae[k]

            for i in xrange(self.n_updrafts):
                GMV.W.values[k] += self.UpdVar.W.values[i,k]*self.UpdVar.Area.values[i,k]
                GMV.H.values[k] += self.UpdVar.H.values[i,k]*self.UpdVar.Area.values[i,k]
                GMV.QT.values[k] += self.UpdVar.QT.values[i,k]*self.UpdVar.Area.values[i,k]
                GMV.T.values[k] += self.UpdVar.T.values[i,k]*self.UpdVar.Area.values[i,k]
        return
        ...
        with nogil:
            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                ...
                for i in xrange(self.n_updrafts):
                    ...
                    self.EnvVar.TKE.press[k] += 0.0*(we_half - wu_half) * (press_buoy + press_drag)
        return
    ...
    cpdef compute_covariance(self, GridMeanVariables GMV, CasesBase Case, TimeStepping TS):
        ...
        if self.calc_tke:
            ...
            self.compute_covariance_entrainment(GMV, GMV.W ,GMV.W , self.EnvVar.TKE, self.UpdVar.TKE, self.UpdVar.W,self.UpdVar.W, self.EnvVar.W, self.EnvVar.W)
            ...
            self.compute_upd_covariance_shear(GMV, self.UpdVar.TKE, self.UpdVar.W, self.UpdVar.W)
            self.compute_tke_pressure()
            self.compute_upd_tke_pressure()
        if self.calc_scalar_var:
            self.compute_covariance_entrainment(GMV, GMV.H ,GMV.H , self.EnvVar.Hvar, self.UpdVar.Hvar, self.UpdVar.H,self.UpdVar.H, self.EnvVar.H, self.EnvVar.H)
            self.compute_covariance_entrainment(GMV, GMV.QT ,GMV.QT , self.EnvVar.QTvar, self.UpdVar.QTvar, self.UpdVar.QT,self.UpdVar.QT, self.EnvVar.QT, self.EnvVar.QT)
            self.compute_covariance_entrainment(GMV, GMV.H ,GMV.QT , self.EnvVar.HQTcov, self.UpdVar.HQTcov, self.UpdVar.H,self.UpdVar.QT, self.EnvVar.H, self.EnvVar.QT)
            ...
            self.compute_upd_covariance_shear(GMV, self.UpdVar.Hvar, self.UpdVar.H, self.UpdVar.H)
            self.compute_upd_covariance_shear(GMV, self.UpdVar.QTvar, self.UpdVar.QT, self.UpdVar.QT)
            self.compute_upd_covariance_shear(GMV, self.UpdVar.HQTcov, self.UpdVar.H, self.UpdVar.QT)
        ...
        if self.calc_tke:
            self.update_covariance_ED(GMV, Case,TS, GMV.W, GMV.W, GMV.TKE, self.EnvVar.TKE, self.UpdVar.TKE, self.EnvVar.W, self.EnvVar.W, self.UpdVar.W, self.UpdVar.W)
        ...
        if self.calc_scalar_var:
            self.update_covariance_ED(GMV, Case,TS, GMV.H, GMV.H, GMV.Hvar, self.EnvVar.Hvar, self.UpdVar.Hvar, self.EnvVar.H, self.EnvVar.H, self.UpdVar.H, self.UpdVar.H)
            self.update_covariance_ED(GMV, Case,TS, GMV.QT,GMV.QT, GMV.QTvar, self.EnvVar.QTvar, self.UpdVar.QTvar, self.EnvVar.QT, self.EnvVar.QT, self.UpdVar.QT, self.UpdVar.QT)
            self.update_covariance_ED(GMV, Case,TS, GMV.H, GMV.QT, GMV.HQTcov, self.EnvVar.HQTcov, self.UpdVar.HQTcov, self.EnvVar.H, self.EnvVar.QT, self.UpdVar.H, self.UpdVar.QT)
    ...
    cpdef cleanup_covariance(self, GridMeanVariables GMV):
        ...
        with nogil:
            for k in xrange(self.Gr.nzg):
                ...
                for i in xrange(self.n_updrafts):
                    if self.UpdVar.Hvar.values[i,k] < tmp_eps:
                        self.UpdVar.Hvar.values[i,k] = 0.0
                    if self.UpdVar.TKE.values[i,k] < tmp_eps:
                        self.UpdVar.TKE.values[i,k] = 0.0
                    if self.UpdVar.QTvar.values[i,k] < tmp_eps:
                        self.UpdVar.QTvar.values[i,k] = 0.0
                    if fabs(self.UpdVar.HQTcov.values[i,k]) < tmp_eps:
                        self.UpdVar.HQTcov.values[i,k] = 0.0
    ...
    cdef void compute_covariance_entrainment(self, GridMeanVariables GMV, VariablePrognostic GmvVar1,VariablePrognostic GmvVar2, EDMF_Environment.EnvironmentVariable_2m EnvCovar, EDMF_Updrafts.UpdraftVariable_2m UpdCovar, EDMF_Updrafts.UpdraftVariable UpdVar1,
        ...
        for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
            EnvCovar.entr[k] = 0.0
            ComVar = ae[k]*EnvCovar.values[k]
            for i in xrange(self.n_updrafts):
                ComVar += self.UpdVar.Area.values[i,k]*UpdCovar.values[i,k]
            ...
            if UpdCovar.name =='tke':
                ...
                meanvar2 = interp2pt(GmvVar2.values[k], GmvVar2.values[k-1])
                tke_factor = 0.5
            else:
                envvar1 = EnvVar1.values[k]
                envvar2 = EnvVar2.values[k]
                meanvar1 = GmvVar1.values[k]
                meanvar2 = GmvVar2.values[k]
                tke_factor = 1.0
            for i in xrange(self.n_updrafts):
                if UpdCovar.name =='tke':
    ...
    cdef void update_covariance_ED(self, GridMeanVariables GMV, CasesBase Case,TimeStepping TS, VariablePrognostic GmvVar1, VariablePrognostic GmvVar2,
        ...
        self.get_GMV_CoVar(self.UpdVar.Area, UpdVar1, UpdVar2, EnvVar1, EnvVar2, Covar, UpdCovar, &GmvVar1.values[0], &GmvVar2.values[0], &GmvCovar.values[0])
...
## ---------- updraft coavriances
            ...
            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                if self.UpdVar.Area.values[i,k] >self.minimum_area:
                    if Covar.name == 'tke':
                        ...
                        du_high = (GMV.U.values[k+1] - GMV.U.values[k]) * self.Gr.dzi
                        dv_high = (GMV.V.values[k+1] - GMV.V.values[k]) * self.Gr.dzi
                        diff_var2 = (UpdVar2.values[i,k] - UpdVar2.values[i,k-1]) * self.Gr.dzi
                        diff_var1 = (UpdVar1.values[i,k] - UpdVar1.values[i,k-1]) * self.Gr.dzi
                        tke_factor = 0.5
                    else:
                        du_low = 0.0
                        dv_low = 0.0
                        du_high = 0.0
                        dv_high = 0.0
                        diff_var2 = interp2pt((UpdVar2.values[i,k+1] - UpdVar2.values[i,k]),(UpdVar2.values[i,k] - UpdVar2.values[i,k-1])) * self.Gr.dzi
                        diff_var1 = interp2pt((UpdVar1.values[i,k+1] - UpdVar1.values[i,k]),(UpdVar1.values[i,k] - UpdVar1.values[i,k-1])) * self.Gr.dzi
                        tke_factor = 1.0
                    with nogil:
                ...
                for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                    if self.UpdVar.Area.values[i,k] >self.minimum_area:
                        UpdCover.dissipation[i,k] = (self.Ref.rho0_half[k] * self.UpdVar.Area.values[i,k] * UpdCover.values[i,k]
                                            *pow(fmax(self.UpdVar.TKE.values[i,k],0), 0.5)/fmax(self.upd_mixing_length[i,k],1.0) * self.tke_diss_coeff)
                    else:
    ...
    cdef void update_upd_covariance_ED(self, GridMeanVariables GMV, CasesBase Case,TimeStepping TS, VariablePrognostic GmvVar1, VariablePrognostic GmvVar2,
        ...
        cdef:
            ...
            double [:] c = np.zeros((nz,),dtype=np.double, order='c')
            double [:] x = np.zeros((nz,),dtype=np.double, order='c')
            double [:] rho_au_K_m = np.zeros((nzg,),dtype=np.double, order='c')
            double [:] whalf = np.zeros((nzg,),dtype=np.double, order='c')
        ...
        for i in xrange(self.n_updrafts):
            ...
            with nogil:
                for k in xrange(1,nzg-1):
                    rho_au_K_m[k] = 0.5 * (self.UpdVar.Area.values[i,k]*self.UpdVar.KH.values[i,k]+ self.UpdVar.Area.values[i,k+1]*self.UpdVar.KH.values[i,k+1])* self.Ref.rho0[k]
