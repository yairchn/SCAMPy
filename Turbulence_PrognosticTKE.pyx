...
cdef class EDMF_PrognosticTKE(ParameterizationBase):
    ...
    cpdef compute_entrainment_detrainment(self, GridMeanVariables GMV, CasesBase Case, TimeStepping TS):
        ...
        #upd 6
        les_z0 = [360.0 ,380.0 ,400.0 ,420.0 ,440.0 ,460.0 ,480.0 ,500.0 ,520.0 ,540.0 ,560.0 ,580.0 ,600.0 ,620.0 ,640.0 ,660.0 ,680.0 ,700.0]
        les_eps0 = [0.0003899911928086195, 0.001526768337266473, 0.0013917949068827456, 0.0000000000000000000, 0.0000000000000000000, 0.0004622622982797022,
                    0.0000000000000000000, 0.0003710831639789813, 0.0000000000000000000, 0.0003610294430165011, 0.00011823124380382103, 0.0000000000000000000,
                    0.0000000000000000000, 0.0022266330230653743, 0.0025904934262534138, 0.0027347017008465483, 0.006582675635171169, 0.005943315781511446]
        les_del0 = [0.0015694133367157538, 0.0016777028873082517, 0.0003103778021552948, 0.000000000000000000, 0.000000000000000000, 0.000000000000000000,
                   0.000000000000000000, 0.0004093536918550226, 0.0009579814640555624, 0.0021016924588745456, 0.0027213906383746103, 0.005279121668855001,
                     0.006238385485354787, 0.009113002050929821, 0.01474718621161078, 0.014791145070335965, 0.02180377039939706, 0.03368236709764953]
        ...
        for i in xrange(self.n_updrafts):
            ...
            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                if TS.t>6*3600.0 and self.Gr.z_half[k]<np.max(z) and self.Gr.z_half[k]>np.min(z):
    ...
    cpdef compute_nh_pressure(self, TimeStepping TS):
        ...
        # updraft 6
        les_z = np.array([360.0 ,380.0 ,400.0 ,420.0 ,440.0 ,460.0 ,480.0 ,500.0 ,520.0 ,540.0 ,560.0 ,580.0 ,600.0 ,620.0 ,640.0 ,660.0 ,680.0 ,700.0])
        dpdz_upd = np.array([0.0010036436012025142, 0.00043755871743457307 ,-0.00036655045809403356 ,-0.0002731455580729581 ,-9.603655778901287e-05 ,
                             0.00025587661648994854 ,0.000748924901885651 ,0.0006777905510599929 ,0.0008856314236130309 ,0.0014623518945573114 ,
                             0.002106278961088867 ,0.002624376147155129 ,0.003260567215215981 ,0.00375987325623796 ,0.004262949729083212 ,0.005101527375250588 ,
                             0.005788588007916446 ,0.005188543752134087])
        # updraft 7
