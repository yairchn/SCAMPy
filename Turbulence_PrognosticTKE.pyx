...
cdef class EDMF_PrognosticTKE(ParameterizationBase):
    ...
    cpdef compute_prognostic_updrafts(self, GridMeanVariables GMV, CasesBase Case, TimeStepping TS):
        ...
        while time_elapsed < TS.dt:
            ...
            self.nan_stopper(GMV, 620)
            self.UpdVar.set_values_with_new()
            self.nan_stopper(GMV, 621)
            ...
            self.nan_stopper(GMV, 643)
            self.decompose_environment(GMV, 'values')
            self.nan_stopper(GMV, 645)
            self.EnvThermo.saturation_adjustment(self.EnvVar)
            self.UpdThermo.buoyancy(self.UpdVar, self.EnvVar, GMV, self.extrapolate_buoyancy)
            self.set_subdomain_bcs()
    ...
    cpdef compute_mixing_length(self, double obukhov_length, double ustar, GridMeanVariables GMV):
        ...
        if (self.mixing_scheme == 'sbl'):
            ...
            for k in xrange(gw, self.Gr.nzg-gw):
                ...
                if obukhov_length < 0.0: #unstable
                    # l2 = vkb * z_ /(sqrt((self.EnvVar.TKE.values[self.Gr.gw]+0.000001)/ustar/ustar)*self.tke_ed_coeff) * fmin(
                    #  (1.0 - 100.0 * z_/obukhov_length)**0.2, 1.0/vkb )
                    l2 = vkb * z_ /(sqrt(self.EnvVar.TKE.values[self.Gr.gw]/ustar/ustar)*self.tke_ed_coeff) * fmin(
                     (1.0 - 100.0 * z_/obukhov_length)**0.2, 1.0/vkb )
                else: # neutral or stable
                    # l2 = vkb * z_ /(sqrt((self.EnvVar.TKE.values[self.Gr.gw]+0.000001)/ustar/ustar)*self.tke_ed_coeff)
                    l2 = vkb * z_ /(sqrt(self.EnvVar.TKE.values[self.Gr.gw]/ustar/ustar)*self.tke_ed_coeff)
    ...
    cpdef compute_eddy_diffusivities_tke(self, GridMeanVariables GMV, CasesBase Case):
        ...
        if self.similarity_diffusivity:
            ...
            if (self.EnvVar.TKE.values[self.Gr.gw])==0.0:
    ...
    cpdef compute_horizontal_eddy_diffusivities(self, GridMeanVariables GMV):
        ...
        with nogil:
            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                for i in xrange(self.n_updrafts):
                    with gil:
                        amin2 = self.minimum_area**2
                        limiter = 1.0 + 10.0*exp(-self.UpdVar.W.values[i,k]**2/(2*amin2))
                    # if self.UpdVar.Area.values[i,k]>0.0:
                    self.horizontal_KM[i,k] = self.UpdVar.Area.values[i,k]*self.turbulent_entrainment_factor \
                    *sqrt(fmax(self.EnvVar.TKE.values[k],0.0))*self.pressure_plume_spacing[i]*limiter
    ...
    cpdef compute_turbulent_entrainment(self, GridMeanVariables GMV, CasesBase Case):
        cdef:
            ...
            double ed_mf_ratio, b_upd_full, b_env_full, env_tke_full, w_up

        with nogil:
            amin2 = 0.00001
            for i in xrange(self.n_updrafts):
                for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                    turb_entr_lim =  2.0*exp(-self.UpdVar.W.values[i,k]**2.0/0.01)
                    ...
                    w_up = self.UpdVar.W.values[i,k]
                    with gil:
                        if wu_half>0.0:
                            wu_half = fmax(wu_half, 0.001)
                        else:
                            wu_half = fmax(wu_half,0.001)

                        if we_half>0.0:
                            we_half = fmax(we_half, 0.001)
                        else:
                            we_half = fmax(we_half,0.001)

                        if w_up>0.0:
                            w_up = fmax(w_up, 0.001)
                        else:
                            w_up = fmax(w_up,0.001)

                    if a*wu_half  > 0.0:
                        self.turb_entr_H[i,k]  = (2.0/R_up**2.0)*self.Ref.rho0_half[k] * a * self.horizontal_KH[i,k]  * \
                                                    (self.EnvVar.H.values[k] - self.UpdVar.H.values[i,k]) * turb_entr_lim
                        self.turb_entr_QT[i,k] = (2.0/R_up**2.0)*self.Ref.rho0_half[k]* a * self.horizontal_KH[i,k]  * \
                                                     (self.EnvVar.QT.values[k] - self.UpdVar.QT.values[i,k]) #* turb_entr_lim
                        self.frac_turb_entr[i,k]    = (2.0/R_up**2.0) * self.horizontal_KH[i,k] / wu_half/a #* turb_entr_lim
                    else:
                    ...
                    if a_full > 0.0:
                        ...
                        self.turb_entr_W[i,k]  = (2.0/R_up_full**2.0)*self.Ref.rho0[k] * a_full * K_full  * \
                                                    (self.EnvVar.W.values[k]-self.UpdVar.W.values[i,k]) #* turb_entr_lim
                        self.frac_turb_entr_full[i,k] = (2.0/R_up_full**2.0) * K_full / w_up / a_full
                    else:
    ...
    cpdef compute_entrainment_detrainment(self, GridMeanVariables GMV, CasesBase Case):
        ...
        for i in xrange(self.n_updrafts):
            ...
            for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
                ...
                # if self.UpdVar.Area.values[i,k]>-0.1:
                input.amin = self.minimum_area
        ...
        with nogil:
            for i in xrange(self.n_updrafts):
                ...
                for k in range(gw, self.Gr.nzg-gw):
                    ...
                    diffusion = -self.a_diff*self.Ref.rho0_half[k]*(self.UpdVar.Area.values[i,k+2]
                    ...
                    #     with gil:
                    #         print('==============================================================')
                    #         print('self.UpdVar.Area.new[i,k+1]',self.UpdVar.Area.new[i,k+1])
    ...
    cpdef solve_updraft_scalars(self, GridMeanVariables GMV):
        cdef:
            ...
            double c1, c2, c3, c4, diffusion
            double a_kp ,a_k ,a_km ,entr_w ,detr_w ,B_k, adv ,exch ,buoy ,diffusion_w, w_old, w_new
            eos_struct sa
        ...
        with nogil:
            for i in xrange(self.n_updrafts):
                ...
                for k in xrange(gw+1, self.Gr.nzg-gw):
                    ...
                    diffusion = -self.a_diff*(self.Ref.rho0_half[k+1]*self.UpdVar.Area.values[i,k+1]*self.UpdVar.H.values[i,k+1]
                                 -2.0*self.Ref.rho0_half[k]*self.UpdVar.Area.values[i,k]*self.UpdVar.H.values[i,k]
                                     +self.Ref.rho0_half[k-1]*self.UpdVar.Area.values[i,k-1]*self.UpdVar.H.values[i,k-1])*dzi*dzi
                    self.UpdVar.H.new[i,k] =  (c2 * self.UpdVar.H.values[i,k]  + c3 * self.UpdVar.H.values[i,k-1]
                                               + c4 * H_entr + self.turb_entr_H[i,k] + diffusion)/c1
                    diffusion = -self.a_diff*(self.Ref.rho0_half[k+1]*self.UpdVar.Area.values[i,k+1]*self.UpdVar.QT.values[i,k+1]
                                 -2.0*self.Ref.rho0_half[k]*self.UpdVar.Area.values[i,k]*self.UpdVar.QT.values[i,k]
                                     +self.Ref.rho0_half[k-1]*self.UpdVar.Area.values[i,k-1]*self.UpdVar.QT.values[i,k-1])*dzi*dzi
                    ...
                    # self.UpdVar.QT.new[i,k] = fmax(self.UpdVar.QT.new[i,k],1e-9)
                    ...
                    with gil:
                        ...
                        # if self.UpdVar.QT.new[i,k]<0.0:
                            ...
                            print('------------------ negative QT ------------------')
                            adv = (m_k  * dzi * self.UpdVar.QT.values[i,k]
                                 - m_km * dzi * self.UpdVar.QT.values[i,k-1])/c1
                            detr = m_k * self.detr_sc[i,k]* self.UpdVar.QT.values[i,k]/c1
                            entr = m_k * self.entr_sc[i,k]* QT_entr/c1
                            old = self.Ref.rho0_half[k]   * self.UpdVar.Area.values[i,k] * self.UpdVar.QT.values[i,k] * dti_/c1
                            turb  = self.turb_entr_QT[i,k]/c1
                            # entr_lim = (1.0 + 10.0*exp(-self.UpdVar.Area.values[i,k]**2.0/(2*amin2))
                            #                      - exp(-(1.0-self.UpdVar.Area.values[i,k])**2/(2*amin2)))
                            entr_lim1 =  0.4*exp(     -self.UpdVar.Area.values[i,k] **2.0/(2*amin2))
                            entr_lim2 =  1.0 - exp(-(1.0-self.UpdVar.Area.values[i,k])**2.0/(2*amin2))
                            detr_lim1 =  0.4*exp(-(1.0-self.UpdVar.Area.values[i,k])**2.0/(2*amin2))
                            detr_lim2 =  1.0 - exp(     -self.UpdVar.Area.values[i,k] **2.0/(2*amin2))
                            w_up = interp2pt(self.UpdVar.W.values[i,k-1], self.UpdVar.W.values[i,k])
                            b_up = interp2pt(self.UpdVar.B.values[i,k-1], self.UpdVar.B.values[i,k])
                            w_en = interp2pt(self.EnvVar.W.values[k-1]  , self.EnvVar.W.values[k])
                            b_en = interp2pt(self.EnvVar.B.values[k-1]  , self.EnvVar.B.values[k])
                            ...
                            logistic_e = 1.0/(1.0+exp(-10.0/0.004*db/dw*(0.25 - self.UpdVar.Area.values[i,k])))
                            logistic_d = 1.0/(1.0+exp( 10.0/0.004*db/dw*(0.25 - self.UpdVar.Area.values[i,k])))
                            mu = self.entrainment_sigma/self.entrainment_scale
                            logistic_e = 1.0/(1.0+exp(-mu*db/dw*(self.updraft_mixing_frac - self.UpdVar.Area.values[i,k]
                                            /(self.UpdVar.Area.values[i,k]+self.EnvVar.Area.values[k]))))
                            logistic_d = 1.0/(1.0+exp( mu*db/dw*(self.updraft_mixing_frac - self.UpdVar.Area.values[i,k]
                                            /(self.UpdVar.Area.values[i,k]+self.EnvVar.Area.values[k]))))
                            ...
                            adv_w = (self.Ref.rho0[k] * a_k * self.UpdVar.W.values[i,k] * self.UpdVar.W.values[i,k] * dzi
                                   - self.Ref.rho0[k-1] * a_km * self.UpdVar.W.values[i,k-1] * self.UpdVar.W.values[i,k-1] * dzi)
                            exch = (self.Ref.rho0[k] * a_k * self.UpdVar.W.values[i,k]
                                    * (entr_w * self.EnvVar.W.values[k] - detr_w * self.UpdVar.W.values[i,k] ) + self.turb_entr_W[i,k])
                            buoy = self.Ref.rho0[k] * a_k * B_k
                            diffusion_w = -self.a_diff*(self.Ref.rho0[k+1]*a_kp*self.UpdVar.W.values[i,k+1]
                                        -2.0*self.Ref.rho0[k]*a_k*self.UpdVar.W.values[i,k]
                                            +self.Ref.rho0[k-1]*a_km*self.UpdVar.W.values[i,k-1])*dzi*dzi
                            w_old = self.Ref.rho0[k] * a_k * self.UpdVar.W.values[i,k]
                            w_new = ( w_old* dti_-adv_w + exch + buoy + self.nh_pressure[i,k]
                                                + self.Ref.rho0[k] * a_k * self.sponge_W[i,k] + diffusion_w)/(self.Ref.rho0[k] * anew_k * dti_)
                        if self.UpdVar.QT.new[i,k]<0.0:
                            print('k=',k)
                            print('z=',self.Gr.z_half[k])
                            print('w_values = ',self.UpdVar.W.values[i,k])
                            print('rho*a*w_values/dt = ',w_old* dti_)
                            print('w_new = ',w_new)
                            print('rho*a*w_new/dt = ',self.Ref.rho0[k] * anew_k *w_new* dti_)
                            print('diffusion_w',diffusion_w)
                            print('adv_w',-adv_w)
                            print('exch',exch)
                            print('buoy',buoy)
                            print('self.nh_pressure[i,k]',self.nh_pressure[i,k])
                            print('self.sponge_W[i,k]',self.Ref.rho0[k] * a_k * self.sponge_W[i,k])
                            print('dw', dw)
                            print('db', db)
                            print('inv_timescale', inv_timescale)
                            print('inv_timescale/dw',inv_timescale/dw)
                            print('RH_upd',self.UpdVar.RH.values[i,k])
                            print('RH_env',self.EnvVar.RH.values[k])
                            print('logistic_e',logistic_e)
                            print('logistic_d',logistic_d)
                            print('moisture_deficit_e', self.sorting_function[i,k])
                            print('moisture_deficit_d', self.b_mix[i,k])
                            print('E*qt',m_k * self.entr_sc[i,k]* QT_entr)
                            print('entr_sc',self.entr_sc[i,k])
                            print('D*qt',m_k * self.detr_sc[i,k]* self.UpdVar.QT.values[i,k])
                            print('detr_sc',self.detr_sc[i,k])
                            print('QT_entr', QT_entr)
                            print('m_k',m_k)
                            print('QT.new'   , self.UpdVar.QT.new[i,k])
                            print('RHS sum'  ,old -adv + entr - detr + turb + diffusion)
                            print('RHS terms',old ,adv , entr , detr, turb, diffusion)
                            ...
                            plt.figure('b/w')
                            plt.plot(np.divide(self.UpdVar.B.values[i,:],self.UpdVar.W.values[i,:]),self.Gr.z, 'r',  linestyle='solid', marker='o')
                            plt.figure('entr')
                            plt.plot(self.entr_sc[i,:],self.Gr.z_half, 'r',  linestyle='solid', marker='o')
                            plt.figure('detr')
                            plt.plot(self.detr_sc[i,:],self.Gr.z_half, 'r',  linestyle='solid', marker='o')
                            plt.figure('w')
        ...
        for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
            for i in xrange(self.n_updrafts):
                if np.isnan(self.UpdVar.Area.values[i,k]*self.UpdVar.W.values[i,k]*self.UpdVar.B.values[i,k]*self.UpdVar.H.values[i,k]
                    *self.UpdVar.QT.values[i,k]*self.UpdVar.T.values[i,k]*self.UpdVar.QL.values[i,k]
