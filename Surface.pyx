...
cdef class SurfaceBase:
    ...
    cpdef free_convection_windspeed(self, GridMeanVariables GMV):
        ...
        zi = get_inversion(&theta_rho[0], &GMV.U.values[0], &GMV.V.values[0], &self.Gr.z[0], kmin, kmax, self.Ri_bulk_crit)
...
cdef class SurfaceFixedFlux(SurfaceBase):
    ...
    cpdef update(self, GridMeanVariables GMV):
        ...
        if GMV.H.name == 'thetal':
            ...
            self.rho_hflux = entropy_flux(rho_tflux/self.Ref.rho0[gw],self.rho_qtflux/self.Ref.rho0[gw],
                                          self.Ref.p0[gw], GMV.T.values[gw], GMV.QT.values[gw])
        self.bflux = buoyancy_flux(self.shf, self.lhf, GMV.T.values[gw], GMV.QT.values[gw],self.Ref.alpha0[gw]  )
        ...
        if not self.ustar_fixed:
            ...
            self.ustar = compute_ustar(self.windspeed, self.bflux, self.zrough, self.Gr.z[gw])
...
cdef class SurfaceFixedCoeffs(SurfaceBase):
    ...
    cpdef update(self, GridMeanVariables GMV):
        ...
        self.rho_qtflux = -self.cq * windspeed * (GMV.QT.values[gw] - self.qsurface) * self.Ref.rho0[gw]
        self.lhf = lv * self.rho_qtflux

        if GMV.H.name == 'thetal':
            self.rho_hflux = -self.ch * windspeed * (GMV.H.values[gw] - self.Tsurface/exner_c(self.Ref.Pg)) * self.Ref.rho0[gw]
            self.shf = cp_  * self.rho_hflux
        elif GMV.H.name == 's':
            self.rho_hflux =  -self.ch * windspeed * (GMV.H.values[gw] - self.s_surface) * self.Ref.rho0[gw]
            pv = pv_star(GMV.T.values[gw])
            pd = self.Ref.p0[gw] - pv
        ...
        self.rho_uflux = - self.Ref.rho0[gw] *  self.ustar * self.ustar / windspeed * GMV.U.values[gw]
        self.rho_vflux = - self.Ref.rho0[gw] *  self.ustar * self.ustar / windspeed * GMV.V.values[gw]
        return
    cpdef free_convection_windspeed(self, GridMeanVariables GMV):
        SurfaceBase.free_convection_windspeed(self, GMV)
        return
