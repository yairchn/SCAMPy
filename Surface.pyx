...
import cython
from thermodynamic_functions cimport latent_heat, cpm_c, exner_c, qv_star_t, sd_c, sv_c, pv_star, theta_rho_c, t_to_entropy_c, t_to_thetali_c
    ...
    cpdef free_convection_windspeed(self, GridMeanVariables GMV):
        ...
        with nogil:
            for k in xrange(self.Gr.nzg):
                ...
                theta_rho[k] = theta_rho_c(self.Ref.p0_c[k], GMV.T.values[k], GMV.QT.values[k], qv)
        zi = get_inversion(&theta_rho[0], &GMV.U.values[0], &GMV.V.values[0], &self.Gr.z_c[0], kmin, kmax, self.Ri_bulk_crit)
...
cdef class SurfaceFixedFlux(SurfaceBase):
    ...
    cpdef update(self, GridMeanVariables GMV):
            ...
            if self.windspeed < 0.1:  # Limit here is heuristic
                if self.bflux > 0.0:
                   self.free_convection_windspeed(GMV)
                    ...
                    print('self.Ref.alpha0_f[gw-1] ==>',self.Ref.alpha0_f[gw-1])
        ...
        self.rho_uflux = - self.Ref.rho0_f[gw-1] *  self.ustar * self.ustar / self.windspeed * GMV.U.values[gw]
        self.rho_vflux = - self.Ref.rho0_f[gw-1] *  self.ustar * self.ustar / self.windspeed * GMV.V.values[gw]
        return
    cpdef free_convection_windspeed(self, GridMeanVariables GMV):
...
cdef class SurfaceFixedCoeffs(SurfaceBase):
    ...
    cpdef update(self, GridMeanVariables GMV):
        ...
        if GMV.H.name == 'thetal':
            self.rho_hflux = -self.ch * windspeed * (GMV.H.values[gw] - self.Tsurface/exner_c(self.Ref.Pg)) * self.Ref.rho0_f[gw-1]
            self.shf = cp_  * self.rho_hflux
        elif GMV.H.name == 's':
            self.rho_hflux =  -self.ch * windspeed * (GMV.H.values[gw] - self.s_surface) * self.Ref.rho0_f[gw-1]
            pv = pv_star(GMV.T.values[gw])
            pd = self.Ref.p0_c[gw] - pv
        ...
        self.rho_uflux = - self.Ref.rho0_f[gw-1] *  self.ustar * self.ustar / windspeed * GMV.U.values[gw]
        self.rho_vflux = - self.Ref.rho0_f[gw-1] *  self.ustar * self.ustar / windspeed * GMV.V.values[gw]
        return
    cpdef free_convection_windspeed(self, GridMeanVariables GMV):
        SurfaceBase.free_convection_windspeed(self, GMV)
        return
...
cdef class SurfaceMoninObukhov(SurfaceBase):
    ...
    cpdef update(self, GridMeanVariables GMV):
        ...
        cdef:
            Py_ssize_t k, gw = self.Gr.gw
            double zb = self.Gr.z_c[gw]
            double theta_rho_g = theta_rho_c(self.Ref.Pg, self.Tsurface, self.qsurface, self.qsurface)
            double theta_rho_b = theta_rho_c(self.Ref.p0_c[gw], GMV.T.values[gw], self.qsurface, self.qsurface)
            double Nb2,
            double h_star
            double pv, pd, sv, sd
            double lv = latent_heat(GMV.T.values[gw])
        ...
        exchange_coefficients_byun(Ri, self.Gr.z_c[gw], self.zrough, &self.cm, &self.ch, &self.obukhov_length)
        self.rho_uflux = -self.cm * self.windspeed * (GMV.U.values[gw] ) * self.Ref.rho0_f[gw-1]
        self.rho_vflux = -self.cm * self.windspeed * (GMV.V.values[gw] ) * self.Ref.rho0_f[gw-1]

        self.rho_hflux =  -self.ch * self.windspeed * (GMV.H.values[gw] - h_star) * self.Ref.rho0_f[gw-1]
        self.rho_qtflux = -self.ch * self.windspeed * (GMV.QT.values[gw] - self.qsurface) * self.Ref.rho0_f[gw-1]
        self.lhf = lv * self.rho_qtflux
