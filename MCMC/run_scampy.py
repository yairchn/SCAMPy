...
class forward_scampy(object):
    ...
    def run(self, theta):
        ...
        paramlist = MCMC_paramlist(theta, self.case_name)
...
def generate_G(theta, new_data):
    ...
    CF =          np.array(new_data.groups['timeseries'].variables['updraft_cloud_cover'])
    CT =          np.array(new_data.groups['timeseries'].variables['updraft_cloud_top'])
...
def write_file(paramlist, folder):
