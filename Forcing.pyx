...
cdef class ForcingStandard(ForcingBase):
    ...
    cpdef update(self, GridMeanVariables GMV):
        ...
        for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
            ...
            GMV.H.tendencies[k] += self.convert_forcing_prog_fp(self.Ref.p0_c[k],GMV.QT.values[k],
...
#             # Apply large-scale horizontal advection tendencies
#             qv = GMV.QT.values[k] - GMV.QL.values[k]
#             GMV.H.tendencies[k] += self.convert_forcing_prog_fp(self.Ref.p0_c[k],GMV.QT.values[k], qv,
...
cdef class ForcingDYCOMS_RF01(ForcingBase):
    ...
    cpdef calculate_radiation(self, GridMeanVariables GMV):
        ...
        for k in xrange(self.Gr.nzg - 1, -1, -1):
            q_0           += self.kappa * self.Ref.rho0_c[k] * GMV.QL.values[k] * self.Gr.dz
        ...
        for k in xrange(1, self.Gr.nzg + 1):
            q_1           += self.kappa * self.Ref.rho0_c[k - 1] * GMV.QL.values[k - 1] * self.Gr.dz
        ...
        for k in xrange(self.Gr.gw, self.Gr.nzg - self.Gr.gw):
            self.dTdt[k] = - (self.f_rad[k + 1] - self.f_rad[k]) / self.Gr.dz / self.Ref.rho0_c[k] / dycoms_cp
    ...
    cpdef update(self, GridMeanVariables GMV):
        ...
        for k in xrange(self.Gr.gw, self.Gr.nzg-self.Gr.gw):
            # Apply large-scale horizontal advection tendencies
            qv = GMV.QT.values[k] - GMV.QL.values[k]
            GMV.H.tendencies[k]  += self.convert_forcing_prog_fp(self.Ref.p0_c[k],GMV.QT.values[k], qv, GMV.T.values[k], self.dqtdt[k], self.dTdt[k])
            GMV.QT.tendencies[k] += self.dqtdt[k]
            # Apply large-scale subsidence tendencies
            GMV.H.tendencies[k]  -= (GMV.H.values[k+1]-GMV.H.values[k]) * self.Gr.dzi * self.subsidence[k]
            GMV.QT.tendencies[k] -= (GMV.QT.values[k+1]-GMV.QT.values[k]) * self.Gr.dzi * self.subsidence[k]
