...
def plot_closures(data, les,tmin, tmax,  title, folder="plots/output/"):
    ...
    plt.subplot(2,3,5)
    plt.plot(np.nanmean(data["updraft_RH"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-", color="royalblue",  label="upd_RH", linewidth = 2)
    plt.plot(np.nanmean(data["env_RH"][:,t_start:t_end],axis=1), data["z_half"]/1000.0, "-",     color="darkorange", label="env_RH", linewidth = 2)
    plt.xlabel("buoyancy [m/s^2]")
    plt.grid(True)
    plt.legend()
    plt.subplot(2,3,6)
